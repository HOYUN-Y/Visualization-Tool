...
| **C9** | **Korea 시군구 버블맵 좌표 오배치** (2026-07-17 신규) | ❌ **실버그** — `mapMode.jsx` `MUN_LATLON`이 **시군구명만으로** 좌표 조회(`MUN_LATLON[r.name]`). 동명 구가 객체 리터럴에 중복(`북구` = 부산·광주 → 뒤가 이김). 샘플 데이터에 `북구` 3개(부산/대구/광주)·`서구` 3개 → **세 도시 버블이 한 좌표에 겹침**, 부산 북구가 광주에 표시(약 200km). esbuild `duplicate-object-key` 경고로 발견 | T2 | 데이터에 이미 `province`가 있으므로 키를 `province+name`으로. 좌표 테이블도 시도별로 분리 필요(동명 구가 5개 시도에 존재) |
...
> **상태:** 결정 전. 코드 착수 금지. 이 절은 결정해야 할 것과 그 전제를 기록한다.
> **현재 원칙 (2026-07-17 사용자 확인):** **로컬에서 잘 도는 게 1순위. 당분간 단독 사용.** 아래는 전부 그 이후의 이야기이며, 로컬 사용성을 해치면서까지 앞당기지 않는다.
> **상업화:** 계획 없음.
...
### 13.2 선행 질문 — 회원이 무엇을 위한 것인가 (미답)

사용자가 물은 것("전용 페이지 vs 관리자 계정 로그인")은 **UI 형태**이고, 그건 아래 목적이 정해진 뒤에야 답할 수 있다. 목적에 따라 필요한 백엔드 규모가 10배 차이난다.

| 목적 | 실제로 필요한 것 | 무게 |
|---|---|---|
| **A. 접근 제한만** — "아무나 내 URL 못 열게" | 로그인 UI 불필요. Cloudflare Access·Basic Auth·IP 제한으로 **앱 코드 0줄** | 극소 |
| **B. 기기 간 내 프로젝트 동기화** — 집/회사에서 같은 프로젝트 | 인증 + 서버 저장소. IndexedDB → 서버 동기화, 충돌 해결(B1과 직결) | 대 |
| **C. 여러 사용자가 각자 프로젝트 보유** | B + 사용자별 격리 + 가입·비밀번호 재설정·탈퇴 | 대 |
| **D. 협업 — 프로젝트 공유·권한** | C + 권한 모델 + 공유 UI. 현재 공유링크(P10)가 부분 대체 중 | 특대 |

**지금 필요한 건 사실상 A일 가능성이 높다** — 단독 사용이고 상업화 계획이 없으므로. A는 앱을 건드리지 않고 호스팅 레이어에서 끝나며, local-first를 유지한다.

### 13.3 결정 대기 항목 — 회원관리 UI 형태

> §13.2에서 **C 이상**을 택한 경우에만 의미가 있다. A/B면 이 절은 폐기된다.
...
**현시점 권고:** 결정을 **미룬다.** 사용자 수가 1이면 둘 다 과잉이고, 실제 사용자가 생기면 그때 규모가 답을 정해준다. 조기 결정은 필요 없는 백엔드를 낳는다.
...
| # | 미결정 | 결정 시점 |
|---|---|---|
| 1 | §13.2 목적 A~D 중 무엇인가 | 배포처(Cloudflare/AWS) 확정 시 |
| 2 | 전용 페이지 vs 관리자 계정 (§13.3) | §13.2가 C 이상일 때만 |
| 3 | 데이터를 서버에 둘 것인가 (§13.4-5) | §13.2와 동시 |
