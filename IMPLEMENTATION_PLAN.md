...
> **Status:** Core v2 **완료** (`v2.0.0`, 2026-07-12) · T1 하드닝 1차 (2026-07-17) · **실사 백로그 소탕 (2026-07-18): 정확성 E1~E6·구조 F1~F6·전역리렌더 C1·formula 한글 B3′ 전부 해소.** 잔여는 전부 조건부 대기(배포 A3′·규모 A5/B1/B4/D1·품질 C6/C7/C8) — [§12 백로그](#12-하드닝-백로그-core-v2-이후-잔여)
> **Updated:** 2026-07-18
...
| ID | 항목 | 확인된 현재 상태 (2026-07-17) | 티어 | 완화책 |
|---|---|---|---|---|
| ~~**C1**~~ | ~~전역 리렌더 store~~ | ✅ **해소 (2026-07-18, 2차 — 선행조건 갖춰 재시도).** `useStore`→`useSyncExternalStore`(Object.is로 바뀐 슬라이스 구독자만 리렌더 + effect-subscribe 누락 버그 해소). **1차 시도가 드러낸 은닉 의존성 2종을 아키텍처로 봉합**: 파생 읽기(`getActiveData`)와 전역 리스트(`NODE.datasets`)를 각각 `useActiveData(id)`(clean[id]+datasetsRev 구독)·`useDatasets()`(datasetsRev 구독) 훅으로 캡슐화하고 **render-path 호출 전수 교체**(13파일·getActiveData 18곳·seedRows 9곳·NODE.datasets 5곳; 콜백은 유지). `datasetsRev` 신호 신설(register/remove/hydrate가 bump, 비영속). pivot `pvState` fresh-object selector → `pvSpec`+useMemo. **staleness E2E**(`cleanReactivity` 3 — clean-mode 자체·비-clean 모드 StatusBar·데이터셋 리스트, 훅 우회 시 실패 실증) + `mapCleanPipeline`(1차가 잡은 회귀, 이제 통과) + `storeRerender` 1. Node 359·E2E 71·verify:dist 9모드 | — | — |
...
| ~~**E2**~~ | ~~`insightEngine` 프로파일이 컬럼을 조용히 자름~~ | ✅ **해소 (2026-07-17)** — 왜도는 O(n)이라 **캡 제거**(이미 캡 없이 도는 O(n log n) 이상치 스캔보다 싸다). 상관은 O(k²)라 캡 유지하되 **truncation을 명시**("first 6 of N … open Stats › Correlation for the full matrix"). `IE`에 dual-mode export 추가 → 이 파일 최초의 테스트(순수 summarize\* 8개) + E2E `profileCoverage` 2(8번째 컬럼 도달·고지 출현, 소규모선 미출현) | — | — |
| ~~**E3**~~ | ~~왜도/첨도 정의가 파일마다 다름~~ | ✅ **해소 (2026-07-17)** — Q-Q 패널의 **표시** 왜도/첨도를 `SM`(G1/G2)로 통일(Descriptive·Profile과 동일). JB **통계량**은 모집단 적률이 정의상 맞으므로 `jb.statistic`은 그대로 — 표시 두 값만 통일. 정규성 판정·문구도 `dispSkew` 사용 | — | — |
| ~~**E4**~~ | ~~`timeSeries.acf`가 결측을 압축~~ | ✅ **해소 (2026-07-17)** — 값을 **위치 보존** + lag별 **pairwise deletion**(양쪽 유한한 쌍만 합산, 밀지 않음). 결측 없을 때 기존 편향추정량과 정확히 동일(무회귀 테스트 잠금). 실측: period-4 계절성이 결측 1개에 0.90→**0.79→0.84**로 안정화되고 결측 수와 무관해짐. `pacf`가 acf 호출해 자동 혜택. `tests/timeSeries.test.js` +3 | — | — |
| ~~**E5**~~ | ~~`logistic`의 "수렴"은 사실이 아님~~ | ✅ **해소 (2026-07-17, 최소 조치)** — gradient max-norm이 `1e-6` 미만이면 조기 종료(진짜 최적점, 더 싸다) + `converged`/`iterationsUsed` 반환. 미수렴 시 ML 패널에 배너("계수·확률은 잠정, 정확도·AUC는 무영향"). **결과 무회귀**: 수렴 후엔 예산 늘려도 가중치 불변(테스트 잠금). IRLS 전환은 큰 범위라 별도. `tests/logistic.test.js` +3(수렴/미수렴/무드리프트) + E2E `logitConvergence` 2(분리→배너·중첩→무배너) | — | — |
| ~~**E6**~~ | ~~`sqlFallback`의 `LIKE`가 `*`를 안 이스케이프~~ | ✅ **해소 (2026-07-17)** — 이스케이프 문자셋에 `*` 추가. `LIKE 'a*'`가 이제 리터럴 "a*"에 매칭(이전엔 `^a*$`="a 0개 이상"). %·_ 와일드카드는 이스케이프 이후 치환이라 무영향. `tests/sqlFallback.test.js` +1 | — | — |
...
- **XLSX 대용량 사용 시작 시**: D1(인메모리 복제 가드) → B4(undo 스택 상한). *C1(전역 리렌더)은 ✅ 해소(2차 재시도).*
