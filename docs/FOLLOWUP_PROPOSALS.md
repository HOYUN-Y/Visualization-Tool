...
> **갱신: 2026-07-12 (5차)** — **§0-0b ML 크래시 수정 검증 + Phase 4 DuckDB 전환(S1~S3) 검증 + Playwright E2E 13/13**. `feat/duckdb` 기준. 이전 이력: 4차(Phase 3.5 게이트)·3차(P0 발견)·2차(P4~P8)는 §6.
...
> ✅ = 처리 + 재검증 통과 (§0-0·§6). **P0~P9·P11 완료 + Phase 4 DuckDB 완주. 남은 것: main/feat/duckdb 병합·push(CLI) → P13(ML 적격성) → P10 잔여·P12 → §5 잠재 리스크**.
...
| P10 | Planned 잔여 (DT/NB/CV·~~SQL JOIN~~·공유링크·PPT 매핑) | 기능 | 대 | ★★ | 부분 — **SQL JOIN/window는 Phase 4 DuckDB로 해소**(§0-0d). DT/NB/CV·공유링크·PPT 매핑만 잔여 |
...
> Phase 3.5 게이트 + §0-0b ML 수정 + Phase 4 DuckDB(S1~S3) + P11 E2E까지 전부 소화됨. 남은 순서:

1. **`feat/duckdb` → main 병합·태그** — 게이트·E2E 13/13 통과. `--no-ff` + 태그(`v2.0.0` 후보 — DuckDB 전환 완료 지점). 원격 push는 별도 외부전송 승인.
2. **P13 (ML 데이터 적격성 검증, §7)** — Logistic 반복 알림의 근본 해결. `mlMode.jsx` 같은 파일이라 §0-0b 후속으로 자연스러움. 착수 권장 1순위.
3. **§5 "몇 줄 수정" 4건** — beforeunload 저장(B2) · ECharts SRI(A3) · export 대상 명시(C4) · (B3는 SQL 주경로 해소됐으니 formula/폴백만 잔여).
