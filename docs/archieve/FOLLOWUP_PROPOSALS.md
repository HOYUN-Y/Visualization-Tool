# 후속작업 제안 (Follow-up Proposals) — 🗄️ 폐기·흡수됨 (2026-07-17)

> # ⛔ 이 문서를 참조하지 마세요
>
> **2026-07-17에 분해되어 정본 문서로 흡수됐습니다.** 아래 내용은 흡수 시점의 스냅샷이며 **더 이상 갱신되지 않습니다.**
>
> | 원래 내용 | 현재 위치 |
> |---|---|
> | 완료 항목 24건 (P0~P13, A1, B2, C2, C4 등) | [`WORKLOG.md`](../../WORKLOG.md) § 완료 항목 원장 |
> | 미완 항목 14건 (§5 잠재 리스크 레지스터) | [`IMPLEMENTATION_PLAN.md`](../../IMPLEMENTATION_PLAN.md) §12 하드닝 백로그 |
>
> **계획의 정본은 [`IMPLEMENTATION_PLAN.md`](../../IMPLEMENTATION_PLAN.md) 하나입니다.** 이 문서는 다른 모델(Fable)이 작성한 *제안* 문서였고, 계획 문서가 두 갈래로 갈려 낡은 클론의 세션이 이미 완료된 작업을 재구현하려던 사고의 원인 중 하나였습니다.
>
> §12로 이월된 항목의 상태 표기는 **2026-07-17 코드 실사로 재확인**한 것이라, 이 문서의 서술과 다를 수 있습니다. 그 경우 §12가 맞습니다.
...
| P10 | Planned 잔여 (~~DT/NB/CV~~·~~SQL JOIN~~·~~공유링크~~·~~PPT 매핑~~) | 기능 | 대 | ★★ | ✅ **완료** — **DT/NB/CV ✅**(§0-0e), **SQL JOIN ✅DuckDB**(§0-0d), **공유링크 ✅**(`feat/p10-share-link`), **PPT 매핑 ✅**(`feat/p10-pptx-chart-mapping` — 스택·보조축·콤보 네이티브 매핑, 캔들스틱 정직한 폴백, Node 329·E2E 28) |
...
1. ~~**PPT 네이티브 차트 매핑 확장** (스택/보조축/캔들)~~ ✅ **완료**(`feat/p10-pptx-chart-mapping`) — 렌더된 ECharts 옵션을 읽어 스택(`barGrouping:"stacked"`)·보조축(`secondaryValAxis`)·콤보(멀티타입 `addChart`)를 PPT 네이티브로 매핑. 캔들스틱·분산형은 대응 형식이 없어 `unsupported`로 정직하게 이미지/SVG 폴백. `js/pptxExport.js` 순수 `planChart` + `tests/pptxExport.test.js`(14) + `tests/e2e/pptxExport.spec.mjs`.
...
4. ~~**공유 링크**~~ ✅ **완료**(`feat/p10-share-link`) — 전체 프로젝트를 `#p=` URL fragment(deflate+base64url, 백엔드 없음)로 공유, 부팅 시 자동 임포트. A1 안전파서 선행 완료로 코드실행 위험 없음. `js/shareLink.js`(`window.ShareLink`) + `tests/shareLink.test.js`(8) + `tests/e2e/shareLink.spec.mjs`.
